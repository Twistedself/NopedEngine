...
function loadMemoryScan_thread(t)
  ...
  t.synchronize(function(t)
	  ...
	  dialog.Filter=translate("Noped Engine Scan files").." (*.CS)|*.CS"
...
s.LoadFromFile(getnopedEngineDir()..[[autorun\images\export128x128.png]])
...
local s=createPicture()
s.LoadFromFile(getnopedEngineDir()..[[autorun\images\import128x128.png]])
local ii=MainForm.mfImageList.add(s.Bitmap)
